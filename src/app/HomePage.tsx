...
export default function HomePage() {
  const { user, isPowerUser, loading: authLoading } = useAuth();
  ...
  useEffect(() => {
    if (user && isPowerUser) {
      setLoading(true);
      Promise.all([fetchFeedbacks(1), fetchErrorLogs()]).then(() => {
        setLoading(false);
      });
    }
  }, [user, isPowerUser, fetchFeedbacks]);
